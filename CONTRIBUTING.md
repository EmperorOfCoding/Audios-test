...
4. **Cheque marcadores automaticamente**: rode `./check-conflicts.sh` para garantir que nenhum marcador de merge (`<<<<<<<`, `=======`, `>>>>>>>`) permaneceu no repositório.
5. **Formate e teste**: após resolver, confira se a estrutura do HTML continua válida e se os timers e players funcionam ao recarregar a página no navegador local.
6. **Marque como resolvido**: use `git add <arquivo>` para cada arquivo corrigido e verifique novamente com `git status`.
7. **Continue o rebase ou crie o commit**: finalize com `git rebase --continue` (em caso de rebase) ou `git commit` se estiver em uma resolução manual.
